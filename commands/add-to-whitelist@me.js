...
exports.run = async (client, message, args, level) => {
  ...
  try {
    ...
    return message.reply(`There was an error adding items to your whitelist for \`${producer}\` in server \`${message.guild.name}\`. Please try again later.`);
  }
  ...
  try {
    ...
    if (modifyFilterResponse) return message.reply(`Successfully added ${newWhitelistItems.length} items to your whitelist for \`${producer}\` in server \`${message.guild.name}\`.`);
  } catch (e) {
...
exports.help = {
  ...
  description: `Adds one or more games or categories to a user's mentions feed whitelist for the specified streamer. The streamer must be included in the user's mentions feed in the given server.`,
  usage: '!add-to-whitelist@me [streamer] smb1 Super_Mario_World sm64|120_Star Super_Mario_Sunshine|Any%'
};